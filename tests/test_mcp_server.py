...
async def test_scan_chart_url(mock_extract_images, mock_requests_get):
    ...
    mock_requests_get.assert_called_once_with("http://example.com/chart.tgz", stream=True)
...
def test_main_helm_cli_not_installed(mock_parse_args, mock_check_helm_cli, mock_mcp_run):
    ...
    with mock.patch("sys.exit") as mock_exit, mock.patch("builtins.print") as mock_print:
        main()
